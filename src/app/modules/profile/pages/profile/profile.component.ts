...
@Component({
  ...
  viewProviders: [
    provideIcons({ lucideUser, lucideLock, lucideMonitor, lucideSave, lucideShieldCheck, lucideSmartphone, lucideLogOut }),
  ],
  template: `
    <div class="p-6 max-w-3xl mx-auto space-y-6">
      ...
      @if (profile()) {
        <div class="flex items-center gap-4 p-4 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm">
          <div class="w-16 h-16 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-2xl font-bold text-indigo-600 dark:text-indigo-400">
            {{ profile()!.firstName.charAt(0) }}{{ profile()!.lastName.charAt(0) }}
          </div>
          <div>
            <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ profile()!.firstName }} {{ profile()!.lastName }}</p>
          ...
          <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6">
            ...
            <form [formGroup]="personalForm" (ngSubmit)="savePersonalInfo()" class="space-y-4">
              ...
              <div class="space-y-1">
                ...
                <input hlmInput type="email" formControlName="email" class="w-full" [disabled]="true" />
                <p class="text-xs text-gray-400 mt-1">Email cannot be changed here. Contact support.</p>
              </div>
              <div class="space-y-1">
              ...
              <div class="flex justify-end pt-2">
                <button hlmBtn type="submit" [disabled]="personalForm.pristine || personalForm.invalid" class="flex items-center gap-2">
        ...
        <div hlmTabsContent="security" class="mt-6 space-y-4">
          ...
          <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6">
            ...
            <form [formGroup]="passwordForm" (ngSubmit)="changePassword()" class="space-y-4">
              <div class="space-y-1">
                ...
                <input hlmInput type="password" formControlName="currentPassword" class="w-full" placeholder="••••••••" />
              </div>
              <div class="space-y-1">
                <label hlmLabel>New Password</label>
                <input hlmInput type="password" formControlName="newPassword" class="w-full" placeholder="••••••••" />
              </div>
              <div class="space-y-1">
                <label hlmLabel>Confirm New Password</label>
                <input hlmInput type="password" formControlName="confirmPassword" class="w-full" placeholder="••••••••" />
              </div>
              @if (passwordMismatch()) {
                <p class="text-sm text-red-500">Passwords do not match.</p>
              }
              <div class="flex justify-end pt-2">
                <button hlmBtn type="submit" [disabled]="passwordForm.invalid || passwordMismatch()" class="flex items-center gap-2">
          ...
          <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6">
            <div class="flex items-center justify-between">
              ...
              <hlm-switch
                [checked]="mfaEnabled()"
                (checkedChange)="toggleMfa($event)">
              </hlm-switch>
            </div>
            @if (mfaEnabled()) {
              <div class="mt-3 p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
        ...
        <div hlmTabsContent="devices" class="mt-6">
          <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">These devices are currently logged into your account.</p>
            </div>
                ...
                <div class="flex items-center justify-between px-6 py-4">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center">
                      <ng-icon [name]="session.device.includes('iPhone') || session.device.includes('Android') ? 'lucideSmartphone' : 'lucideMonitor'"
                        size="20" class="text-gray-500 dark:text-gray-400">
                      </ng-icon>
                    </div>
                    <div>
                      <div class="flex items-center gap-2">
                      ...
                      <p class="text-xs text-gray-400">{{ session.location }} &middot; Last active {{ session.lastActive | date:'mediumDate' }}</p>
                    </div>
                  </div>
                  @if (!session.isCurrent) {
                    <button hlmBtn variant="ghost" size="sm" class="text-red-500 hover:text-red-700 flex items-center gap-1 text-xs">
