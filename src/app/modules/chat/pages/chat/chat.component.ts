...
@Component({
  ...
  viewProviders: [
    provideIcons({ lucideSend, lucideSearch, lucideMessageSquare, lucideCircle, lucidePaperclip, lucideSmile }),
  ],
  template: `
    <div class="flex h-[calc(100vh-4rem)] overflow-hidden bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm m-6">
        ...
        <div class="flex-1 overflow-y-auto">
          @for (conv of filteredConversations(); track conv.conversationId) {
            <button
              ...
              [class.dark:bg-indigo-900/20]="activeConversation()?.conversationId === conv.conversationId">

              <!-- Avatar -->
              <div class="relative flex-shrink-0">
                <div class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-semibold text-sm">
                  {{ getInitials(conv) }}
                </div>
                @if (isOnline(conv)) {
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between">
                  <span class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ getConvName(conv) }}</span>
                  @if (conv.lastMessage) {
                    <span class="text-xs text-gray-400 flex-shrink-0 ml-2">{{ conv.lastMessage.createdAt | date:'shortTime' }}</span>
                  }
                </div>
                @if (conv.lastMessage) {
                  <p class="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{{ conv.lastMessage.content }}</p>
                }
              </div>

              <!-- Unread Badge -->
              @if (conv.unreadCount > 0) {
                <span class="flex-shrink-0 w-5 h-5 rounded-full bg-indigo-600 text-white text-xs flex items-center justify-center">
                  {{ conv.unreadCount }}
                </span>
              }
            </button>
      ...
      <div class="flex-1 flex flex-col min-w-0">
        ...
        @if (activeConversation()) {
          ...
          <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-3">
            <div class="relative">
              <div class="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-semibold text-sm">
            ...
            <div>
              <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ getConvName(activeConversation()!) }}</p>
              <p class="text-xs text-gray-400">{{ isOnline(activeConversation()!) ? 'Online' : 'Offline' }}</p>
            </div>
          </div>
          ...
          <div #messageList class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            @for (msg of messages(); track msg.messageId) {
              <div class="flex gap-3" [class.flex-row-reverse]="msg.senderId === currentUserId">
                <div class="w-8 h-8 rounded-full flex-shrink-0 bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-xs font-medium text-gray-600 dark:text-gray-300">
                  {{ msg.senderName.charAt(0) }}
                </div>
                <div class="max-w-[70%] space-y-1" [class.items-end]="msg.senderId === currentUserId">
                  <div
          ...
          <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/20">
            <div class="flex items-end gap-2">
              ...
              <button hlmBtn size="icon" class="flex-shrink-0 mb-0.5" (click)="sendMessage()" [disabled]="!messageText.trim()">
          ...
          <div class="flex-1 flex flex-col items-center justify-center text-center px-8">
            ...
            <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-1">No conversation selected</h3>
            <p class="text-sm text-gray-400">Choose a conversation from the left panel to start chatting.</p>
          </div>
        }
...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  sendMessage(): void {
    ...
    this.chatService.sendMessage({ conversationId: this.activeConversation()!.conversationId, content: text }).subscribe(msg => {
