...
import {
  lucideInbox, lucideSend, lucideFileEdit, lucideAlertTriangle, lucideTrash2,
  lucideStar, lucideSearch, lucidePencil, lucideRefreshCw, lucideChevronLeft,
  lucideChevronRight, lucideMoreVertical, lucideReply, lucideForward,
  lucideTag, lucideX, lucideCheck, lucideLoader, lucidePaperclip,
    ...
    <div class="flex h-full -m-6 bg-background">
        ...
        <nav class="flex-1 px-2 pb-2 space-y-0.5">
          <button
            *ngFor="let tab of tabs"
            ...
            [ngClass]="activeCategory() === tab.key
          ...
          <!-- Labels -->
          <div class="pt-3">
            <p class="px-3 text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Labels</p>
            <div *ngFor="let label of labels()" class="flex items-center gap-2 px-3 py-1.5 rounded-md cursor-pointer
      ...
      <div class="w-80 border-r border-border flex flex-col shrink-0">
        ...
        <div class="p-3 border-b border-border">
          <div class="relative">
            <ng-icon name="lucideSearch" class="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <input
              [(ngModel)]="searchQuery"
        ...
        <div class="flex items-center justify-between px-4 py-2 border-b border-border">
          ...
          <button (click)="refresh()" class="p-1 rounded text-muted-foreground hover:text-foreground">
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <div *ngIf="!isLoading() && filteredEmails().length === 0" class="text-center text-muted-foreground p-8 text-sm">
          ...
          >
            ...
            <div class="flex items-center gap-1.5">
              <ng-icon *ngIf="email.isStarred" name="lucideStar" class="w-3.5 h-3.5 text-amber-400 shrink-0 fill-amber-400" />
              <span class="text-sm truncate"
            ...
            <div *ngIf="email.labels?.length" class="flex gap-1 mt-1.5 flex-wrap">
              <span *ngFor="let label of email.labels"
        ...
        <div class="border-t border-border px-4 py-2 flex items-center justify-between">
          <span class="text-xs text-muted-foreground">1–{{ filteredEmails().length }} of {{ totalCount() }}</span>
          <div class="flex gap-1">
            <button class="p-1 rounded hover:bg-accent text-muted-foreground disabled:opacity-40" [disabled]="page() <= 1" (click)="prevPage()">
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <ng-container *ngIf="composeOpen(); else emailDetail">
          <div class="flex items-center justify-between px-6 py-4 border-b border-border">
            ...
            <button (click)="composeOpen.set(false)" class="p-1.5 rounded-md hover:bg-accent text-muted-foreground">
          ...
          <div class="flex-1 flex flex-col p-6 gap-3">
            <div class="flex items-center gap-3 border-b border-border pb-3">
              ...
              <input [(ngModel)]="composeTo" type="email" placeholder="recipient@example.com"
            ...
            <div class="flex items-center gap-3 border-b border-border pb-3">
              ...
              <input [(ngModel)]="composeSubject" type="text" placeholder="Email subject"
                class="flex-1 text-sm bg-transparent focus:outline-none text-foreground" />
            </div>
            <textarea
              [(ngModel)]="composeBody"
            ...
            <div class="flex items-center gap-2 pt-3 border-t border-border">
              ...
              <button (click)="composeOpen.set(false)" class="ml-auto p-2 rounded-md hover:bg-accent text-muted-foreground">
        ...
        <ng-template #emailDetail>
          <!-- Empty state -->
          <div *ngIf="!selectedEmail()" class="flex-1 flex flex-col items-center justify-center text-muted-foreground">
          ...
          <ng-container *ngIf="selectedEmail() as email">
            ...
            <div class="flex items-center gap-2 px-6 py-3 border-b border-border">
              <button (click)="selectedEmail.set(null)" class="p-1.5 rounded-md hover:bg-accent text-muted-foreground md:hidden">
                <ng-icon name="lucideChevronLeft" class="w-4 h-4" />
              </button>
              <div class="flex items-center gap-2 ml-auto">
                <button (click)="toggleStar(email)" class="p-1.5 rounded-md hover:bg-accent text-muted-foreground">
                  <ng-icon name="lucideStar" class="w-4 h-4"
                    [ngClass]="email.isStarred ? 'fill-amber-400 text-amber-400' : ''" />
                </button>
                <button (click)="replyEmail(email)" class="p-1.5 rounded-md hover:bg-accent text-muted-foreground">
                  <ng-icon name="lucideReply" class="w-4 h-4" />
                </button>
                <button (click)="forwardEmail(email)" class="p-1.5 rounded-md hover:bg-accent text-muted-foreground">
                  <ng-icon name="lucideForward" class="w-4 h-4" />
                </button>
                <button (click)="trashEmail(email)" class="p-1.5 rounded-md hover:bg-accent text-muted-foreground">
                  <ng-icon name="lucideTrash2" class="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-6">
              ...
              <div class="flex items-start gap-3 mb-6">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    <div>
                      <span class="font-medium text-foreground text-sm">{{ email.from.name || email.from.email }}</span>
                      <span class="text-muted-foreground text-xs ml-1">&lt;{{ email.from.email }}&gt;</span>
                    </div>
                    <span class="text-xs text-muted-foreground shrink-0 ml-2">
              ...
              <div *ngIf="email.attachments?.length" class="mt-6 pt-4 border-t border-border">
                ...
                <div class="flex flex-wrap gap-2">
                  <div *ngFor="let att of email.attachments"
...
export class EmailComponent implements OnInit {
  ...
  sendEmail(): void {
    this._emailService
      .sendEmail({ subject: this.composeSubject, body: this.composeBody, to: [{ email: this.composeTo }] })
      .subscribe(() => {
        this.composeOpen.set(false);
  ...
  saveDraft(): void {
    this._emailService
      .saveDraft({ subject: this.composeSubject, body: this.composeBody, to: [{ email: this.composeTo }] })
  ...
  refresh(): void { this._loadEmails(); }
  prevPage(): void { if (this.page() > 1) { this.page.update(p => p - 1); this._loadEmails(); } }
  nextPage(): void { this.page.update(p => p + 1); this._loadEmails(); }
}
