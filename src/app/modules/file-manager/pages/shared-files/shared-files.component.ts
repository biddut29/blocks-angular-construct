...
@Component({
  ...
  viewProviders: [
    provideIcons({
      lucideLayoutGrid, lucideList, lucideFolder, lucideFile,
      lucideFileText, lucideImage, lucideTrash2, lucideChevronRight, lucideShare2,
    }),
  ],
  template: `
      ...
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <nav class="flex items-center gap-1 mt-1 text-sm text-gray-500 dark:text-gray-400">
            <span class="hover:text-indigo-600 cursor-pointer" (click)="navigateToRoot()">Shared Files</span>
        ...
        <div class="flex items-center gap-2">
          <input hlmInput [(ngModel)]="searchQuery" placeholder="Search shared files..." class="w-48 text-sm" />

          <div class="flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
            <button hlmBtn variant="ghost" class="rounded-none border-r px-3"
              [class.bg-indigo-50]="viewMode() === 'grid'" (click)="viewMode.set('grid')">
              <ng-icon name="lucideLayoutGrid" size="16"></ng-icon>
            </button>
            <button hlmBtn variant="ghost" class="rounded-none px-3"
              [class.bg-indigo-50]="viewMode() === 'list'" (click)="viewMode.set('list')">
            ...
            <tbody cdkDropList (cdkDropListDropped)="onDrop($event)">
              @for (item of filteredFiles(); track item.fileId) {
                <tr cdkDrag
                  ...
                  <td class="px-6 py-3 text-gray-500 text-xs">{{ item.createdAt | date:'mediumDate' }}</td>
                </tr>
              }
            </tbody>
          </table>
...
export class SharedFilesComponent implements OnInit {
  ...
  navigateToRoot(): void {
    ...
    this.fileService.getFiles({ pageNo: 1, pageSize: 50 }).subscribe(res => this.files.set(res.items.filter(f => f.isShared)));
  }
  ...
  navigateTo(folderId: string): void {
    ...
    this.fileService.getFiles({ pageNo: 1, pageSize: 50, parentId: folderId }).subscribe(res => this.files.set(res.items.filter(f => f.isShared)));
  }

  onItemDoubleClick(item: FileItem): void {
    if (item.type === 'folder') {
      this.breadcrumbs.update(list => [...list, { id: item.fileId, name: item.name }]);
      this.fileService.getFiles({ pageNo: 1, pageSize: 50, parentId: item.fileId }).subscribe(res => this.files.set(res.items.filter(f => f.isShared)));
    }
  }
