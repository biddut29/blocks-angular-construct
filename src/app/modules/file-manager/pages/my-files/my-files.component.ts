// ─── My Files Component ────────────────────────────────────────────────────────
// Mirrors: src/modules/file-manager/pages/MyFiles in React project
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, DragDropModule, NgIconComponent, HlmButton, HlmInput],
  viewProviders: [
    provideIcons({
      lucideLayoutGrid, lucideList, lucideUpload, lucideFolderPlus,
      lucideFolder, lucideFile, lucideFileText, lucideImage,
      lucideTrash2, lucideMoreVertical, lucideChevronRight,
    }),
  ],
  template: `
      ...
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <nav class="flex items-center gap-1 mt-1 text-sm text-gray-500 dark:text-gray-400">
            <span class="hover:text-indigo-600 cursor-pointer" (click)="navigateToRoot()">My Files</span>
            @for (crumb of breadcrumbs(); track crumb.id) {
              <ng-icon name="lucideChevronRight" size="12"></ng-icon>
              <span class="hover:text-indigo-600 cursor-pointer" (click)="navigateTo(crumb.id)">{{ crumb.name }}</span>
            }
          </nav>
        </div>
        ...
        <div class="flex items-center gap-2">
          <!-- Search -->
          <input hlmInput [(ngModel)]="searchQuery" placeholder="Search files..." class="w-48 text-sm" />

          <!-- View Toggle -->
          <div class="flex rounded-md border border-gray-200 dark:border-gray-700 overflow-hidden">
            <button
              hlmBtn variant="ghost"
              class="rounded-none border-r border-gray-200 dark:border-gray-700 px-3"
          ...
          <!-- New Folder -->
          <button hlmBtn variant="outline" class="flex items-center gap-2 text-sm" (click)="createFolder()">
            <ng-icon name="lucideFolderPlus" size="16"></ng-icon>
            New Folder
          </button>

          <!-- Upload -->
          <button hlmBtn class="flex items-center gap-2 text-sm" (click)="triggerUpload()">
            <ng-icon name="lucideUpload" size="16"></ng-icon>
            Upload
          </button>
        ...
        <div
          ...
          @for (item of filteredFiles(); track item.fileId) {
            <div
              ...
              <!-- Icon -->
              <div class="w-14 h-14 flex items-center justify-center rounded-lg" [class]="getIconBg(item)">
                <ng-icon [name]="getFileIcon(item)" size="28" [class]="getIconColor(item)"></ng-icon>
              </div>
              <!-- Name -->
              <span class="text-xs text-gray-700 dark:text-gray-300 font-medium text-center truncate w-full">{{ item.name }}</span>
              @if (item.size) {
              ...
              @for (item of filteredFiles(); track item.fileId) {
                <tr cdkDrag
                  ...
                  <td class="px-6 py-3 flex items-center gap-3">
                    ...
                    <span class="font-medium text-gray-800 dark:text-gray-200 truncate max-w-xs">{{ item.name }}</span>
                  </td>
                  <td class="px-6 py-3 text-gray-500 dark:text-gray-400 capitalize">{{ item.type }}</td>
                  <td class="px-6 py-3 text-gray-500 dark:text-gray-400">{{ item.size ? formatSize(item.size) : '—' }}</td>
                  <td class="px-6 py-3 text-gray-500 dark:text-gray-400 text-xs">{{ item.createdAt | date:'mediumDate' }}</td>
                  <td class="px-6 py-3 text-right">
                    <button (click)="deleteFile(item); $event.stopPropagation()" class="text-gray-400 hover:text-red-500 transition-colors">
...
export class MyFilesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.fileService.getFiles({ pageNo: 1, pageSize: 50, parentId: folderId ?? undefined }).subscribe(res => {
  ...
  navigateToRoot(): void {
    ...
    this.fileService.getFiles({ pageNo: 1, pageSize: 50 }).subscribe(res => this.files.set(res.items));
  }
  ...
  navigateTo(folderId: string): void {
    ...
    this.fileService.getFiles({ pageNo: 1, pageSize: 50, parentId: folderId }).subscribe(res => this.files.set(res.items));
  }

  onItemDoubleClick(item: FileItem): void {
    if (item.type === 'folder') {
      this.breadcrumbs.update(list => [...list, { id: item.fileId, name: item.name }]);
      this.fileService.getFiles({ pageNo: 1, pageSize: 50, parentId: item.fileId }).subscribe(res => this.files.set(res.items));
    }
  }
  ...
  createFolder(): void {
    ...
    if (!name) return;
    this.fileService.createFolder({ name }).subscribe(folder => {
      this.files.update(list => [folder, ...list]);
    });
  }
  ...
  onFileSelected(event: Event): void {
    ...
    Array.from(input.files ?? []).forEach(file => {
      this.fileService.uploadFile({ file }).subscribe(item => {
