...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, BaseChartDirective, NgIconComponent, HlmCard, HlmCardHeader, HlmCardTitle, HlmCardContent],
  viewProviders: [
    provideIcons({
      lucideDollarSign,
      lucideUsers,
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Welcome back! Here is what is happening today.</p>
        </div>
      </div>
        ...
        @for (card of metricCards(); track card.title) {
          <div hlmCard class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm">
            <div hlmCardHeader class="flex flex-row items-center justify-between px-6 pt-6 pb-2">
              <p hlmCardTitle class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ card.title }}</p>
              <div class="p-2 rounded-lg bg-indigo-50 dark:bg-indigo-900/30">
                <ng-icon [name]="card.icon" class="text-indigo-600 dark:text-indigo-400" size="18"></ng-icon>
              </div>
            </div>
            <div hlmCardContent class="px-6 pb-6">
              ...
              <div class="flex items-center gap-1 mt-1">
                ...
                <span [class]="card.change >= 0 ? 'text-green-600 text-xs font-medium' : 'text-red-600 text-xs font-medium'">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6">
          <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-4">Product Categories</h2>
          <div class="relative h-64">
            <canvas baseChart
              [data]="pieChartData"
        ...
        <!-- Quick Stats -->
        <div class="lg:col-span-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6">
          ...
          <div class="space-y-4">
            @if (stats()) {
              <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-700">
                ...
                <span class="text-sm font-semibold text-gray-900 dark:text-white">\${{ stats()!.totalRevenue | number }}</span>
              </div>
              <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-700">
                <span class="text-sm text-gray-600 dark:text-gray-400">Total Users</span>
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ stats()!.totalUsers | number }}</span>
              </div>
              <div class="flex items-center justify-between py-3 border-b border-gray-100 dark:border-gray-700">
                <span class="text-sm text-gray-600 dark:text-gray-400">Total Orders</span>
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ stats()!.totalOrders | number }}</span>
              </div>
              <div class="flex items-center justify-between py-3">
                <span class="text-sm text-gray-600 dark:text-gray-400">Total Products</span>
                <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ stats()!.totalProducts | number }}</span>
              </div>
            } @else {
  ...
  private _loadStats(): void {
    this.dashboardService.getStats().subscribe(data => {
      ...
      this.metricCards.set([
        { title: 'Total Revenue', value: data.totalRevenue, change: data.revenueChange, icon: 'lucideDollarSign', prefix: '$' },
        { title: 'Total Users', value: data.totalUsers, change: data.usersChange, icon: 'lucideUsers' },
        { title: 'Total Orders', value: data.totalOrders, change: data.ordersChange, icon: 'lucideShoppingCart' },
